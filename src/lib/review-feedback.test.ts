...
import {
  extractReviewFeedbackItems,
  formatReviewFeedbackSummary,
  summarizeReviewFeedback,
  countActionableFeedback,
...
describe("extractReviewFeedbackItems", () => {
  it("filters actionable review feedback and threads", () => {
    ...
    expect(
      items.some((item) => item.author === "alice" && item.source === "review")
    ).toBe(true);
    expect(
      items.some((item) => item.author === "carol" && item.source === "review")
    ).toBe(true);
    expect(
      items.some((item) => item.author === "dave" && item.source === "thread")
    ).toBe(true);
  });

  it("ignores trivial comments", () => {
    const data: GitHubReviewData = {
      reviews: [{ author: "bob", body: "LGTM", state: "APPROVED" }],
      reviewThreads: [],
      comments: [],
    };
  ...
  it("filters feedback by timestamp", () => {
    const items = extractReviewFeedbackItems(sampleData, {
      afterTimestamp: "2026-01-14T12:08:00Z",
    });
  ...
  it("skips unresolved threads before timestamp", () => {
    ...
    const items = extractReviewFeedbackItems(data, {
      afterTimestamp: "2026-01-14T00:00:00Z",
    });
    expect(items).toHaveLength(0);
  });
  ...
  it("filters PR comments by timestamp", () => {
    const data: GitHubReviewData = {
      ...
      comments: [
        ...
        {
          author: "frank",
          body: "Also update that",
          createdAt: "2026-01-14T15:00:00Z",
        },
      ],
    };
    const items = extractReviewFeedbackItems(data, {
      afterTimestamp: "2026-01-14T12:00:00Z",
    });
    expect(items).toHaveLength(1);
...
describe("formatReviewFeedbackSummary", () => {
  ...
  it("formats PR comment summary lines", () => {
    const items = [
      { source: "comment" as const, author: "eve", body: "Please add tests" },
    ];
    const summary = formatReviewFeedbackSummary(items);
...
describe("summarizeReviewFeedback", () => {
  it("returns empty summary when no actionable feedback", () => {
    const result = summarizeReviewFeedback({
      reviews: [],
      reviewThreads: [],
      comments: [],
    });
...
describe("countActionableFeedback", () => {
  ...
  it("returns zero counts for empty data", () => {
    const counts = countActionableFeedback({
      reviews: [],
      reviewThreads: [],
      comments: [],
    });
